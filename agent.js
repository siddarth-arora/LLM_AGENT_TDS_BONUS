/**
 * LLMChatBot - Advanced AI Assistant
...
class LLMChatBot {
    ...
    async init() {
        try {
            ...
            console.log(`LLMChatBot v${this.version} initialized successfully`);
        } catch (error) {
            console.error('Failed to initialize LLMChatBot:', error);
    ...
    displayMessage(message) {
        try {
            ...
            const senderName = { user: 'You', assistant: 'LLMChatBot', system: 'System', tool: 'Tool' }[message.role] || message.role;
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to LLMChatBot! How can I assist you?');
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.agentFlow = new LLMChatBot();
});
